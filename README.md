...
| Fichier | Feature |
|---|---|
| `cpt-exporter-settings.php` | Page de réglages **Réglages > CPT Exporter** : liste les articles et les types de contenu personnalisés. Cocher un type affiche ses champs natifs, ses taxonomies et ses champs ACF, à cocher à leur tour. Le bouton d'enregistrement stocke la sélection dans l'option `cpt_exporter_settings`. |
...
1. **Réglages > CPT Exporter** : cocher un type, puis les champs natifs, taxonomies et champs ACF à exporter. Enregistrer.
2. Sur la liste de ce type, **Export > CSV** ou **Export > XLSX** télécharge `{type}-{AAAA-MM-JJ}.csv` ou `.xlsx`.

Une ligne par contenu publié, numérotée à partir de 1 dans la première colonne (`#`), puis une colonne par champ coché, dans l'ordre de la page de réglages.
...
- **Enregistrement.** Une seule option, `cpt_exporter_settings`, via l'API Settings (`options.php`, nonce et capacité `manage_options` gérés par WordPress). Le nettoyage ne garde que les valeurs que la page propose au moment de l'enregistrement : un type, une taxonomie ou un champ disparu est retiré à la sauvegarde suivante. Décocher un type conserve ses sous-sélections : elles sont masquées, pas effacées, et réapparaissent si on le recoche. L'export ne lit que les types dont `enabled` vaut `true`.
- **Affichage conditionnel.** Les sous-options d'un type sont masquées tant qu'il n'est pas coché, par une seule règle CSS `:has()` écrite dans la page : ni JavaScript, ni fichier de style. Structure enregistrée :
  ...
  ```php
  ['projet' => ['enabled' => true, 'fields' => ['excerpt'], 'taxonomies' => ['category'], 'acf' => ['field_…']]]
  ```
...
Dernier passage : 68/68 sur `feat/cpt-exporter-export`, puis 7/7 sur la numérotation après son ajout.
